...
namespace JobsForCoders.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult JobList(Search search)
        {
            if (Session["LoggedUser"] != null)
            {
                var exists = db.Offres.Include(jp => jp.Entreprise).Where(jp =>
        ...
        //Login for Employeer
        public ActionResult EntrepriseLogin()
        {
            return View();
        }

        [HttpPost]
        public ActionResult EntrepriseLogin([Bind(Include = "Email,Password")] Login login)
        {
            var exists = db.Entreprises.Where(e =>
               e.Email == login.Email
            ...
            if (exists != null)
            {
                ...
                Session["LoggedUserID"] = exists.EntrepriseID;
        ...
        {
            var exists = db.Candidats.Where(js =>
        ...
        public async Task<ActionResult> Apply(int JobID)
        {
            ...
            var jobPosting = db.Offres
                .Include(e => e.Entreprise)
                .Where(e => e.JobID == JobID)
                .FirstOrDefault();

            var employer = jobPosting.Entreprise;

            var jobSeeker = db.Candidats.FirstOrDefault(js => js.JobSeekerID == jobSeekerId);
