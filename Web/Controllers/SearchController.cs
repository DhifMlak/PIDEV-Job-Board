...
namespace JobsForCoders.Controllers
{
    public class SearchController : Controller
    {
        ...
        {
            var searchQuery = db.Offres.Include(jp => jp.Entreprise);

            if (search.City != null)
            {
                searchQuery = searchQuery.Where(jp =>
                    jp.Entreprise.City.Contains(search.City));
            }
            ...
            var employerList = searchQuery.Select(jp => jp.Entreprise).Distinct().ToList();
        ...
        {
            var searchQuery = db.Candidats.AsQueryable();
